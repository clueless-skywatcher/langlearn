...
export default async function DrillPage(
  ...
  const checkpoint = isCheckpoint(section);
  const count = atomicQuestions(section.drills).length;

  return (
    <div>
      <p className="text-xs uppercase tracking-wide text-ink-faint">
        {checkpoint ? "Checkpoint" : "Drill"} · {section.level}
      </p>
      ...
      {checkpoint && (
        <p className="mt-4 rounded border border-accent/50 bg-accent-soft/50 px-4 py-3 text-sm text-ink-soft">
          ...
          answer, <strong>−1</strong> for a wrong single-correct answer, partial
          credit on multiple-correct questions, and <strong>−2</strong> there if
          any wrong option is chosen. Integer answers are never marked negative.
        </p>
      )}
